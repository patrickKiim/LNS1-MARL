...
class CL_MAPFEnv(gym.Env):
    ...
    def joint_step(self, actions):
        ...
        add_rewards = np.zeros((1, self.local_num_agents), dtype=np.float32)
        ...
        for i in range(self.local_num_agents):
            ...
            if reach_goal_status[i] == 1:
                ...
                if actions[i] == 0:
                    x, num_blocking = self.get_blocking_reward(i)
            ...
            add_rewards[:, i]=state[-1]
            obs[:, i, :, :, :] = state[0]
            vector[:, i, : 3] = state[1]
            vector[:, i, 3] = (self.sipps_coll[i] - self.total_coll[i]) / (self.sipps_coll[i] + 1)
        ...
        vector[:, :, 7] = num_on_goal / self.local_num_agents
        vector[:, :, -1] = actions
        done=False
        ...
        return obs, vector, rewards, done, next_valid_actions, blockings, num_blockings, \
            leave_goals, num_on_goal, self.max_on_goal, num_dynamic_collide,num_agent_collide,add_rewards
    ...
    def get_blocking_reward(self, local_agent_index):
        other_agents = []
        top_left = (self.world.local_agents_poss[local_agent_index][0] - self.observation_size // 2,
                    self.world.local_agents_poss[local_agent_index][1] - self.observation_size // 2)
        ...
        for agent in range(self.local_num_agents):
            if agent == local_agent_index:
                continue
            x, y = self.world.local_agents_poss[agent]
