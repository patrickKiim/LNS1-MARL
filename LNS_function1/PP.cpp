...
path_table(instance.map_size), goal_table(instance.map_size, -1)
{
    ...
    for (auto& i:agents) {
        goal_table[i.path_planner.goal_location] = i.id; //debug note: changed fromm ->goal_locatiion to .goal
    }
}
...
{   //neighbor.agents.resize(agents.size());
    ...
    ConstraintTable constraint_table(path_table, instance.num_of_cols, instance.map_size);
