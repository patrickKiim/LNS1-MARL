...
vector<vector<pair<int,int>>> AnytimeEECBS::run()
{
    ...
    int a = 0;
    int p = 0;
    vector<vector<pair<int,int>>> temp(8,vector<pair<int,int>>(10000, make_pair(-1,-1)));
    while (p != num_of_agents)
    {
        int t=0;
        for (const auto &state : solution[p])
        {   temp[a][t]=instance.getCoordinate(state.location);
            t++;
        }
        temp[a].resize(t);
        a++;
        ++p;  // probable error in this line
    }
    return temp;
}
