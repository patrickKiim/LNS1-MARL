...
def runPP(inputWorldFile,inputPosFile,outputFile,num_agent):
    argument = "./LNS_function1_i/function1 -m " + inputWorldFile + " -a " + inputPosFile + " -o " + outputFile + " -n " + str(num_agent)
...
def run_pp(Map,start_list,goal_list,env_id):
    curr_path=os.getcwd()
    inputWorldFile = curr_path+"/record_files_imitation/world"+str(env_id)+".txt"
    inputPosFile = curr_path+"/record_files_imitation/pos"+str(env_id)+".txt"
    outputFile = curr_path+"/record_files_imitation/pp_output"+str(env_id)+".txt"
...
def run_selection(inputWorldFile,inputPosFile,inputpathFile,outputFile,local_num_agents,ALNS,global_num_collison,update_weight,
    ...
    argument = "./LNS_function2_i/function2 -m " + inputWorldFile + " -a " + inputPosFile +" -p " + inputpathFile+" -o " + outputFile + " -n " + str(
...
def check_collision(path,num_agent,map_size,env_id):
    curr_path = os.getcwd()
    inputPathFile = curr_path + "/record_files_imitation/temp_paths" + str(env_id) + ".txt"
    outputFile = curr_path + "/record_files_imitation/check_output" + str(env_id) + ".txt"
    ...
    argument = "./LNS_function3_i/function3 -m " + str(map_size) + " -p " + inputPathFile + " -o " + outputFile + " -n " + str(num_agent)
    with tempfile.TemporaryFile() as tempf:  # generate temp file
...
def adaptive_destroy(temp_path,local_num_agents,ALNS,global_num_collison,
    ...
    inputWorldFile =curr_path+ "/record_files_imitation/world" + str(env_id) + ".txt"
    inputPosFile = curr_path+"/record_files_imitation/pos" + str(env_id) + ".txt"
    inputpathFile=curr_path+"/record_files_imitation/paths" + str(env_id) + ".txt"
    outputFile = curr_path+"/record_files_imitation/selection_output" + str(env_id) + ".txt"
    pathFile = open(inputpathFile, "w")
...
def run_eecbs_intermediate(Map, start_list, goal_list, env_id):

    curr_path=os.getcwd()
    inputWorldFile = curr_path+"/record_files_imitation/world_inter"+str(env_id)+".txt"
    inputPosFile = curr_path+"/record_files_imitation/pos_inter"+str(env_id)+".txt"
    outputFile = curr_path+"/record_files_imitation/pp_output_inter"+str(env_id)+".txt"
    mapFile = open(inputWorldFile, "w")
    posFile = open(inputPosFile, "w")
    writeMap(mapFile, Map)
    writeAgentPos(posFile, start_list,goal_list)
    mapFile.close()
    posFile.close()
    runEECBS_inter(inputWorldFile,inputPosFile,outputFile,len(start_list))
    resultFile = open(outputFile, 'r')

    paths = readResults_inter(resultFile)

    resultFile.close()
    return paths


def readResults_inter(resultFile):
    all_lines=resultFile.readlines()
    paths=[]
    for i,line in enumerate(all_lines):
        # if i==0:
...
def runEECBS_inter(inputWorldFile,inputPosFile,outputFile,num_agent):
    argument = "./LNS_function4/function4 -m " + inputWorldFile + " -a " + inputPosFile + " -o " + outputFile + " -n " + str(num_agent)
    with tempfile.TemporaryFile() as tempf:  # generate temp file
        proc = subprocess.Popen(argument, stdout=tempf, shell=True)  # using shell to execute argment, return completedprocess object to tempf
        proc.wait()  # wait for the processing ned
